...
dpd.addfriend = dpd("/addfriend");
dpd.addfriend.on = function(ev, fn) {
  return dpd.on("addfriend" + ":" + ev, fn);
}
dpd.addfriend.once = function(ev, fn) {
  return dpd.once("addfriend" + ":" + ev, fn);
}
dpd.addfriend.off = function(ev, fn) {
  return dpd.off("addfriend" + ":" + ev, fn);
}

dpd.chatrooms = dpd("/chatrooms");
dpd.chatrooms.on = function(ev, fn) {
  return dpd.on("chatrooms" + ":" + ev, fn);
}
dpd.chatrooms.once = function(ev, fn) {
  return dpd.once("chatrooms" + ":" + ev, fn);
}
dpd.chatrooms.off = function(ev, fn) {
  return dpd.off("chatrooms" + ":" + ev, fn);
}
dpd.chatrooms.rename = function (path, body, fn) {
  dpd.chatrooms.exec("rename", path, body, fn);
}

dpd.deletefriend = dpd("/deletefriend");
dpd.deletefriend.on = function(ev, fn) {
  return dpd.on("deletefriend" + ":" + ev, fn);
}
dpd.deletefriend.once = function(ev, fn) {
  return dpd.once("deletefriend" + ":" + ev, fn);
}
dpd.deletefriend.off = function(ev, fn) {
  return dpd.off("deletefriend" + ":" + ev, fn);
}

dpd.joinroom = dpd("/joinroom");
dpd.joinroom.on = function(ev, fn) {
  return dpd.on("joinroom" + ":" + ev, fn);
}
dpd.joinroom.once = function(ev, fn) {
  return dpd.once("joinroom" + ":" + ev, fn);
}
dpd.joinroom.off = function(ev, fn) {
  return dpd.off("joinroom" + ":" + ev, fn);
}

dpd.leaveroom = dpd("/leaveroom");
dpd.leaveroom.on = function(ev, fn) {
  return dpd.on("leaveroom" + ":" + ev, fn);
}
dpd.leaveroom.once = function(ev, fn) {
  return dpd.once("leaveroom" + ":" + ev, fn);
}
dpd.leaveroom.off = function(ev, fn) {
  return dpd.off("leaveroom" + ":" + ev, fn);
}

dpd.logon = dpd("/logon");
dpd.logon.on = function(ev, fn) {
  return dpd.on("logon" + ":" + ev, fn);
}
dpd.logon.once = function(ev, fn) {
  return dpd.once("logon" + ":" + ev, fn);
}
dpd.logon.off = function(ev, fn) {
  return dpd.off("logon" + ":" + ev, fn);
}
...
dpd.mybucket = dpd("/mybucket");
dpd.mybucket.on = function(ev, fn) {
  return dpd.on("mybucket" + ":" + ev, fn);
}
dpd.mybucket.once = function(ev, fn) {
  return dpd.once("mybucket" + ":" + ev, fn);
}
dpd.mybucket.off = function(ev, fn) {
  return dpd.off("mybucket" + ":" + ev, fn);
}

dpd.roomratings = dpd("/roomratings");
dpd.roomratings.on = function(ev, fn) {
  return dpd.on("roomratings" + ":" + ev, fn);
}
dpd.roomratings.once = function(ev, fn) {
  return dpd.once("roomratings" + ":" + ev, fn);
}
dpd.roomratings.off = function(ev, fn) {
  return dpd.off("roomratings" + ":" + ev, fn);
}
dpd.roomratings.rename = function (path, body, fn) {
  dpd.roomratings.exec("rename", path, body, fn);
}

dpd.rooms = dpd("/rooms");
dpd.rooms.on = function(ev, fn) {
  return dpd.on("rooms" + ":" + ev, fn);
}
dpd.rooms.once = function(ev, fn) {
  return dpd.once("rooms" + ":" + ev, fn);
}
dpd.rooms.off = function(ev, fn) {
  return dpd.off("rooms" + ":" + ev, fn);
}
dpd.rooms.rename = function (path, body, fn) {
  dpd.rooms.exec("rename", path, body, fn);
}

dpd.roomtags = dpd("/roomtags");
dpd.roomtags.on = function(ev, fn) {
  return dpd.on("roomtags" + ":" + ev, fn);
}
dpd.roomtags.once = function(ev, fn) {
  return dpd.once("roomtags" + ":" + ev, fn);
}
dpd.roomtags.off = function(ev, fn) {
  return dpd.off("roomtags" + ":" + ev, fn);
}
dpd.roomtags.rename = function (path, body, fn) {
  dpd.roomtags.exec("rename", path, body, fn);
}

dpd.roomtagsref = dpd("/roomtagsref");
dpd.roomtagsref.on = function(ev, fn) {
  return dpd.on("roomtagsref" + ":" + ev, fn);
}
dpd.roomtagsref.once = function(ev, fn) {
  return dpd.once("roomtagsref" + ":" + ev, fn);
}
dpd.roomtagsref.off = function(ev, fn) {
  return dpd.off("roomtagsref" + ":" + ev, fn);
}
dpd.roomtagsref.rename = function (path, body, fn) {
  dpd.roomtagsref.exec("rename", path, body, fn);
}
