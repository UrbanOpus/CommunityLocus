...
S3Bucket.events = ["upload", "get", "delete", "listFolder"];
...
S3Bucket.prototype.handle = function (ctx, next) {
  ...
  if (req.method === "POST" && !req.internal && req.headers['content-type'].indexOf('multipart/form-data') === 0) {
    ...
    var uploadedFile = function(err, file) {
      if (err) {
        error = err;
        return ctx.done(err);
      } else if (!err) {
...
//          }
  ...
  if (req.method === "POST" || req.method === "PUT") {
    ...
    if(ctx.url && ctx.url.slice(-1) === "/" ){
        if(this.events.listFolder){
        this.events.listFolder.run(ctx, domain, function(err) {
            if (err) return ctx.done(err);
            bucket.listFolder(ctx, next);
        });
        }else{
            bucket.listFolder(ctx, next);
        }
...
S3Bucket.prototype.uploadFile = function(filename, filesize, mime, stream, fn, ctx) {
  ...
  var put = this.client.putStream(stream, ctx.url + filename, headers, function(err, res) {
    if (err) return ctx.done(err);
...
//        var url = 'https://' + bucket.config.bucket + '.s3.amazonaws.com' + ctx.url;
//        var result = {
//            "files":[{
//            "name": filename,
//            "size": filesize,
//            "url": url+filename,
//            "thumbnail_url": url+filename,
//            "delete_url": "http://localhost:2403/mybucket" + ctx.url + filename,
//            "delete_type": "DELETE"
//            }]
//        };
//
//        ctx.done(err,result);
        ...
        var file = {
            ...
            size: filesize,
            url: url+filename,
            thumb: url+"thumb_"+filename
        };

        if(filename.slice(0,6) == "thumb_")
            fn();
        else
            fn(null, file);
    }
...
//  put.on('progress', function(data){
//      ctx.session.socket.emit('progress', data);
//  });
};
...
S3Bucket.prototype.listFolder = function(ctx, next) {
    var folder = ctx.url.slice(1,-1);
    console.log("Listing: " + folder);

    this.client.list({ prefix: folder }, function(err, data){
        ctx.done(err || data);
    });
};
