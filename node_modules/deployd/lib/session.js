...
//
            ...
            //delete socketIndex[sid][socket.id];
//
//            if(sid) {
...
//          console.log(socketIndex[sid][socket.id].id);
...
function Session(data, store, sockets, rawSockets) {
  ...
  this.store = store;
    ...
    //TODO: Don't bind socket to queue
    ...
    emit: function (ev) {
      ...
      if(s) {
        //s.emit.apply(s, arguments);
          rawSockets.in('user' + sid).emit.apply(rawSockets, arguments);
      } else {
        // otherwise add to bind queue
      ...
      join: function (){
          ...
          if(s) {
              ...
              // otherwise add to bind queue
              var queue = this._joinQueue = this._bindQueue || [];
              queue.push(arguments);
          }
      }
  };
  ...
  this.emitToAll = function() {
    ...
    this.emitToRoom = function(room, event, data) {
        rawSockets.in(room).emit.apply(rawSockets, [event, data]);
    };
  ...
  store.socketQueue.once(this.sid, function (socket) {
    ...
    if(socketWrapper._emitQueue && socketWrapper._emitQueue.length) {
      ...
      // drain join queue
      if(socketWrapper._joinQueue && socketWrapper._joinQueue.length) {
          socketWrapper._joinQueue.forEach(function (args) {
              socket.join.apply(socket, args);
          });
      }
  });
}
