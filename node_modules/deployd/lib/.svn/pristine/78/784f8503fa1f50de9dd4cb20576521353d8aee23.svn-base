...
Script.prototype.run = function (ctx, domain, fn) {
  ...
  var scriptContext = {
    ...
    mySocket: session && session.store.socketIndex[session.data.id],
    sid: session && session.data && session.data.id,
    console: console,
    sockets: ctx.server && ctx.server.sockets,
    query: ctx.query,
    ...
    emit: function(collection, query, event, data) {
      if(arguments.length === 4) {
        session.emitToUsers(collection, query, event, data);
          ...
          if(session.emitToAll) session.emitToRoom(collection, query, event);
